...
# Kamus massa atom unsur periodik (1–118)
massa_atom = {
...
# ----------------------------------------------
# Hitung massa segmen & unsur
# ----------------------------------------------
def molar_mass_segment(segment: str, detail_dict=None, multiplier=1):
    """
    Menghitung massa molar dari 1 segmen (tanpa titik)
    dan mengisi detail unsur ke detail_dict jika disediakan.
    """
    stack = [0]
    i = 0
    while i < len(segment):
        if segment[i] in "([{":
            ...
            if segment[i].isupper():
                ...
                if detail_dict is not None:
                    detail_dict[elem] += cnt * multiplier
...
# ----------------------------------------------
# Hitung total dan detail unsur
# ----------------------------------------------
def compute_total_molar_mass(formula: str):
    total = 0
    ...
    for part in formula.split("."):
        ...
        if m:
            ...
            mult = 1
            seg = part
        total += molar_mass_segment(seg, details, multiplier=mult) * mult
...
# Cetak tabel hasil
# ----------------------------------------------
def print_table(raw_formula, normalized_formula, mm, inp, outp, mode, details):
    print("\n" + "-"*45)
    print(f"Rumus input       : {raw_formula}")
    print(f"Rumus normalisasi : {normalized_formula}")
    print(f"Massa molar total : {mm:.5f} g/mol")
    print("\n📘 Rincian unsur dan jumlah atom:")
    for elem, count in sorted(details.items()):
        print(f"  - {elem:<2} : {count}")
    print("-"*45)
    if mode == '1':
...
def main():
    ...
    while True:
        ...
        if mode == '1':
            ...
            grams = mol * mm
            print_table(raw_formula, normalized_formula, mm, mol, grams, '1', details)
        elif mode == '2':
            grams = input_positive_number("Masukkan jumlah gram: ")
            mol = grams / mm
            print_table(raw_formula, normalized_formula, mm, grams, mol, '2', details)
        else:
