...
# ----------------------------------------------
# Fungsi normalisasi huruf senyawa (versi pintar)
# ----------------------------------------------
def normalize_formula(formula: str) -> str:
    """
    Perbaiki penulisan rumus (misal 'cuso4' -> 'CuSO4').
    Menghapus spasi dan mengganti tanda '·' jadi '.'.
    Mengenali unsur dari daftar massa_atom agar lebih akurat.
    """
    formula = formula.replace(" ", "").replace("·", ".")
    formula = formula.lower()  # ubah dulu ke huruf kecil semua
    i = 0
    hasil = ""
    while i < len(formula):
        # Jika huruf dan huruf berikutnya membentuk simbol dua huruf yang valid (misal 'cu', 'fe')
        if i + 1 < len(formula) and (formula[i].upper() + formula[i+1].lower()) in massa_atom:
            hasil += formula[i].upper() + formula[i+1].lower()
            i += 2
        # Jika huruf tunggal valid (misal 'o', 's', 'h', 'n')
